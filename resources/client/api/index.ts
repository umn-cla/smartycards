...
export async function createCard(newCardForm: {
  ...
  const res = await axios.post<{ data: T.Card }>(`/cards`, newCardForm);
  return res.data.data;
}
...
export async function getDeckShareLink(deckId: number) {
  const res = await axios.get<{ url: string }>(`/decks/${deckId}/memberships/share`);
  return res.data.url;
}
