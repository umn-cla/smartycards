    ...
    <div class="max-w-4xl mx-auto py-8 px-4">
      <header class="mb-8">
        ...
        <p class="text-gray-600">
          Choose a deck that students will practice. You can configure additional
          settings after selecting a deck.
        </p>
      </header>

      <div v-if="isLoadingDecks" class="text-center py-12">
        <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-brand-maroon-800"></div>
      ...
      <div v-else>
        ...
        <!-- Deck List -->
        <div v-if="filteredDecks.length === 0" class="text-center py-12 bg-white rounded-lg border border-gray-200">
          <p class="text-gray-600">No decks found. Create a deck first before setting up an LTI assignment.</p>
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <h3 class="font-semibold text-lg text-gray-900">{{ deck.name }}</h3>
                <p v-if="deck.description" class="text-gray-600 text-sm mt-1">
                  {{ deck.description }}
                </p>
                <div class="flex gap-4 mt-2 text-sm text-gray-500">
                  <span>{{ deck.cards_count }} cards</span>
                  <span v-if="deck.is_shared" class="text-brand-teal-600">Shared with you</span>
                </div>
              </div>
              <div v-if="selectedDeck?.id === deck.id" class="ml-4 flex-shrink-0">
                <svg class="w-6 h-6 text-brand-teal-600" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div v-if="selectedDeck" class="mt-8 bg-white rounded-lg border border-gray-200 p-6">
          <h2 class="text-xl font-semibold text-gray-900 mb-4">Assignment Configuration</h2>
...
const submitSelection = async () => {
  ...
  try {
    ...
    titleInput.value = config.value.title || `Practice: ${selectedDeck.value.name}`;
...
const cancel = () => {
  ...
  if (confirm('Are you sure you want to cancel? This will close the assignment setup.')) {
    window.close();
  }
};
</script>
